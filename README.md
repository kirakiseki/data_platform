...
|----------|----------|
| `/` | 系统首页 |
| `/dashboard` | 数据总览看板（全网流量、出行量、速度概况） |
| `/road-status` | 路段路况列表（日期/小时筛选，5 级路况） |
| `/congestion` | 路况地图（Leaflet 可视化，按拥堵级别着色，支持 bbox 分页加载） |
| `/network-status` | 路网整体状态趋势（小时粒度折线图） |
| `/hotspot` | 热点分析（网格热力图/POI 热点/聚类热点/行政区对比） |
| `/hotspot-map` | 热点地图可视化 |
| `/trip-features` | 出行特征（距离分布/时段分布/速度分布） |
| `/trajectory-map` | 轨迹地图（随机采样行程在地图上展示） |
| `/map` | 底图浏览（bfmap_ways 路网渲染） |
